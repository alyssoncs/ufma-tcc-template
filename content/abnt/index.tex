...
O \template{} utiliza biblatex com o estilo \texttt{abnt} para gerenciar as referências do artigo, elas devem ser salvas no arquivo \bibfile, a sintaxe se assemelha ao que está descrito no \autoref{lst:bibtex-syntax}.
...
\begin{listing}[htb]
  ...
  \end{minted}
  \caption{Sintaxe BibTeX}
  \label{lst:bibtex-syntax}
\end{listing}

\subsection{Citação indireta}

Para fazer uma citação indireta, é apenas necessário usar a macro \texttt{\textbackslash{}cite}, passando o id da referência. Como apresentado no \autoref{lst:indirect-citation}.
...
\begin{listing}[htb]
  ...
  \label{lst:indirect-citation}
\end{listing}

O resultado vai ser: Programar pode ser bastante prazeroso~\cite{knuth:aocp:1997}.

Também pode-se usar a macro \texttt{\textbackslash{}textcite} para adicionar o autor sem parênteses, como no \autoref{lst:textcite-example}.

\begin{listing}[htb]
  \begin{minted}{latex}
    De acordo com \textcite{knuth:goto:1974}, otimização prematura é a raiz de todo mal.
  \end{minted}
  \caption{Citação com textcite}
  \label{lst:textcite-example}
\end{listing}

O resultado vai ser: De acordo com \textcite{knuth:goto:1974}, otimização prematura é a raiz de todo mal.
...
\begin{listing}[htb]
  \begin{minted}{latex}
    \begin{citacao}
      ...
      abolished from all \enquote{higher level} programming languages.~\cite{dijkstra:goto:1968}.
    \end{citacao}
  \end{minted}
  \caption{Citação direta}
  \label{lst:direct-citation}
\end{listing}
...
\begin{listing}[htb]
  \begin{minted}{latex}
    \begin{figure}[htb]
      ...
      \fonte{\textcite{al-fuqaha:et-al:2015}}
    \end{figure}
  \end{minted}
  \caption{Adição de imagem}
  \label{lst:figure}
\end{listing}
...
\begin{table}[htb]
  \begin{center}
    \IBGEtab{
      ...
      % O smiley ":)" e proposital; o ")" confunde o balanceamento do ChkTeX
      % (W9/W10) e o espaco antes do ":" dispara W26 -- falsos positivos aqui.
      \fonte{ChatGPT :)} % chktex 9 % chktex 26
    } % chktex 10
  \end{center}
\end{table}
...
Utilize o comando \texttt{\textbackslash{}foreign} para deixar o termo em itálico. Tome como exemplo o \autoref{lst:loanword}, ele gerará o seguinte parágrafo, com a palavra \enquote{\dataset} em itálico:

Para a análise estatística, foi necessário utilizar um \foreign{dataset} abrangente, contendo uma grande quantidade de dados que permitiram gerar insights significativos sobre o comportamento do usuário.

\begin{listing}[htb]
  \begin{minted}{latex}
    Para a análise estatística, foi necessário utilizar um \foreign{dataset} abrangente, contendo uma grande quantidade de dados que permitiram gerar insights significativos sobre o comportamento do usuário.
  \end{minted}
  \caption{Estrangeirismos}
  \label{lst:loanword}
\end{listing}
